...
export function createEventLogger(eventBus: IEventBus): Record<LogLevelString, (message: string) => void> {
  return {
    debug: (message: string) => eventBus.emit(APP_EVENTS.LOG, { level: 'debug', message }),
    info: (message: string) => eventBus.emit(APP_EVENTS.LOG, { level: 'info', message }),
    warn: (message: string) => eventBus.emit(APP_EVENTS.LOG, { level: 'warn', message }),
    error: (message: string) => eventBus.emit(APP_EVENTS.LOG, { level: 'error', message }),
  };
}
