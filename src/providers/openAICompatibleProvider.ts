import { z } from "zod";
import { IChatMessage, IAnonymizer, IPrompt, ILlmProvider, IToolDefinition, ToolCall, IStreamingDeanonymizer, IHttpClient, IHttpResponse, IReasoningProcessor, IReasoningDelta } from "../types.js";
...
const OpenAIStreamDeltaSchema = z.object({
  ...
  /**
   * Optional field for vendor-specific metadata.
   * For Gemma4 via OpenAI API, this can contain 'google' with 'thought' flag.
   */
  extra_content: z.record(z.string(), z.any()).nullish(),
});
...
export class OpenAICompatibleProvider implements ILlmProvider {
  ...
  async query(
    ...
    const processed = this.reasoningProcessor.process(choice);
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (hasContent) {
              ...
              // 1. Content is actually non-empty (processed length > 0)
   ...
   * 
   * @param processed - The processed delta object from the reasoning processor.
