...
describe('ChatPrompt', () => {
  ...
  it('should add context if not present and provided', () => {
    ...
    expect(chatPrompt.generateChatHistory()).toEqual([INITIAL_SYSTEM_MESSAGE, { role: "system", content: context }]);
  });
  ...
  it('should update context if present and new context is provided', () => {
    ...
    expect(chatPrompt.generateChatHistory()).toEqual([INITIAL_SYSTEM_MESSAGE, { role: "system", content: newContext }]);
  });
  ...
  it('should remove context if present and no new context is provided', () => {
    ...
    expect(chatPrompt.generateChatHistory()).toEqual([INITIAL_SYSTEM_MESSAGE, { role: "user", content: "Hi" }]);
  });
  ...
  it('should handle context with user messages only', () => {
    ...
    const chatPrompt = new ChatPrompt(chatHistory, context);
    expect(chatPrompt.generateChatHistory()).toEqual([INITIAL_SYSTEM_MESSAGE, { role: "system", content: context }, { role: "user", content: "Hello" }]);
  });
